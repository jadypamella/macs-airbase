...
const DOMAIN_LABELS: Record<string, { sv: string; en: string }> = {
  OPS:    { sv: 'Uppdrag', en: 'Ops' },
  FUEL:   { sv: 'Bränsle', en: 'Fuel' },
  ARMING: { sv: 'Beväpning', en: 'Arming' },
  MAINT:  { sv: 'Underhåll', en: 'Maint' },
  THREAT: { sv: 'Hotbild', en: 'Threat' },
}

export function EmergenceGraph({ events }: EmergenceGraphProps) {
  const { lang } = useLang()
  const actionCounts = Object.keys(MAC_NAMES).map(id => ({
    name: DOMAIN_LABELS[id]?.[lang] || id,
